...
| 滑动、长按、跨按钮或 `CANCEL` | 取消定时任务并清空按钮、时间、候选动作和第一次触点信息；当前新手势按既有规则继续或结束。 |
...
- Android 不同 API 对 `ViewConfiguration` 双击 Slop 的可用属性可能存在差异，需要在项目最低支持版本确认兼容调用方式。
- `onDragEnd` 当前返回同步 `Action`，等待窗口可能需要增加回调或统一动作派发接口；需在实现前确认不破坏服务进程的动作执行时序。
- “第二击跨按钮”是清理旧状态并按新按钮开始普通手势，还是直接丢弃新手势，需在交互测试中确认；本 SPEC 只要求旧状态不得泄漏。
- 需要补充真机／模拟器时序测试，覆盖快速双击、临界超时、临界 Slop、旋转／进程销毁和不同边缘位置。
