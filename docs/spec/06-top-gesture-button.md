...
架构草案／待实施。本文件只定义跨进程、窗口、手势、设置和备份的实施契约，当前没有任何代码已完成，不能把下文的设计当作已交付功能。

## 复杂度

最高。新增 `Position.Top` 会使所有 `Position` 穷举分支重新编译，同时跨越独立 DataStore、主进程设置页、服务进程窗口、Insets／刘海、方向数学、三种动画、动作浮层和备份格式。必须保持旧用户没有 Top 配置时的行为完全不变，并完成四边回归。

## 问题与目标

当前触钮只有 Left、Right、Bottom 三个位置。顶部边缘缺少独立的触钮入口，无法在状态栏下方提供从上向下进入屏幕的手势。
...
- 顶部窗口避开状态栏、display cutout、横屏安全区和系统手势区域，不以 `FLAG_LAYOUT_NO_LIMITS` 或 `LAYOUT_IN_DISPLAY_CUTOUT_MODE_ALWAYS` 为理由覆盖不可用区域。
- ActionPanel、QuickLauncher、QuickTools、TaskSwitcher、PinnedScreenshot 等动作浮层能够识别 Top 锚点并在安全区内展示。
...
- 为 Top 定义 Insets 感知的窗口布局、命中 Rect、`start/end/width` 几何和系统手势安全边界，覆盖状态栏、cutout、IME、横竖屏和系统手势导航。
...
- 不覆盖状态栏、刘海、display cutout、系统 mandatory gesture 区域，不以固定屏幕像素替代 Insets。
...
3. **编辑语义**：Top 只编辑自身列表项；不显示“复制另一侧”和 side 对齐开关。Top 的颜色、长度、厚度、动作和长滑动作独立保存。顶部的系统手势排除默认关闭，是否显示该选项须由 API 和 Insets 能力确认。
4. **浮层锚点**：Top 触发的动作面板和快捷工具面板从安全区顶部向下展开；快速启动器、任务切换器在顶部安全区内以触点横坐标对齐。面板不能遮盖状态栏、cutout 或自身触钮命中区。
...
- `GestureButton` 的字段格式保持不变。Top 默认 `start/end` 使用现有长度约束，运行时再按可用横向安全区夹取；`alignRegion` 对 Top 固定为 false 或在 UI 中隐藏，不能传播到 side。

### DataStore 与服务快照

- 在 [`DataStoreFiles.kt`](../../app/src/main/java/com/aaron/sidegesture/constant/DataStoreFiles.kt) 增加唯一 `TOP_GESTURE_BUTTONS` 文件名（沿用现有命名序列，例如 `hh`，实施时先确认没有历史占用）。
...
Top 的 Window 使用 `Gravity.LEFT or Gravity.TOP`，但位置必须在 Insets 感知的安全矩形内：

```text
safeLeft   = max(systemBars.left, displayCutout.left, mandatoryGestures.left)
safeRight  = screenWidth - max(systemBars.right, displayCutout.right, mandatoryGestures.right)
safeTop    = max(statusBars.top, displayCutout.top, mandatoryGestures.top)
safeBottom = screenHeight - max(navigationBars.bottom, displayCutout.bottom, mandatoryGestures.bottom)
```

- Top 的 `width = availableWidth * (end - start)`、`height = button.width`、`x = safeLeft + availableWidth * start`、`y = safeTop`。`x + width` 必须夹取到 `safeRight`，不可用区域不足时使用居中夹取并保留最小触摸长度。
- `WindowLayoutParams.updateGestureButton()`、`GestureButton.bounds()` 和 Compose 首页预览必须共用同一套安全矩形，避免窗口命中和 UI 高亮错位。`imePadding` 仅参与 fit／几何计算，不能作为 IME 可见性信号；`bounds()` 必须同时接收独立的 `imeVisible`（或语义等价布尔值）和可表达 Top Insets 的参数。
- Bottom 仍贴 `safeBottom - button.width`，Left／Right 仍按现有侧边规则；Top 的新增分支不能改变三边的历史坐标。

### 生命周期和系统区域

- `GestureWindowManager` 在 `onConfigurationChanged()`、Insets 变化、IME 变化、状态栏／cutout 变化时重新计算 Top Window；横屏时按新的安全宽度重算 `start/end`，不沿用竖屏像素。
- Top 必须服从需求 4 的 `hideGestureOnIme`：当独立的 `imeVisible` 为 true 且该设置开启时，Top Window 立即设为不可见并清除触摸命中；隐藏优先级高于任何 fit／夹取计算，不能把 Top 挪到键盘上方继续命中。浮动键盘、IME 过渡期高度为零或 `imePadding` 尚未更新时，只要 `imeVisible` 为 true 仍必须隐藏。`imePadding` 的数值变化不能单独触发隐藏或恢复。IME 消失后按最新 Insets 恢复原 Top Window 和 `touchEnabled`，不改变 `start/end` 配置。
...
- `setBasic()` 当前使用 `FLAG_LAYOUT_NO_LIMITS` 和始终允许 cutout 的策略。Top 接入前必须证明 Window 坐标仍被安全矩形约束；不能用 flag 绕过 Insets。若系统 API 无法提供 Insets，使用已存在的系统资源 fallback，并把不确定性记录到诊断日志。
- Top 触摸区域与系统 mandatory gesture 重叠时，优先保留系统手势，按 `touchEnabled = false` 或缩短安全区处理；不得默认打开 `excludeSystemGestureRects`。状态栏和 cutout 区域永远不可命中。
...
- [`GestureAnglesScreen.kt`](../../app/src/main/java/com/aaron/sidegesture/ui/screen/gestureangles/GestureAnglesScreen.kt) 增加 Top 导航图标、顶部安全 padding、拖拽圆心和文案。Bottom 的导航栏 padding、半径和“向左／向右／向上”文案特判不得被 Top 复用后反转。
...
- Capsule／Bubble 的进度为 `fingerYAnimVal`，中心横坐标沿 `originXAnimVal → fingerXAnimVal` 轻微跟随，顶部起点从安全矩形上边缘向下进入；不能沿用 Bottom 的 `-fingerYAnimVal`。
- Wave 的方向保护为 `fingerYAnimVal < 0` 时不绘制；贝塞尔路径从顶部安全边缘开始向下，形变轴使用 `originXAnimVal - fingerXAnimVal`，stroke 偏移和 icon 半径按垂直镜像计算。
...
- `safeBounds` 必须同时约束左右横向落点和顶部安全边缘；横屏、cutout 和宽度不足时不能绘制到不可见区域。
...
- [`HomeScreen.kt`](../../app/src/main/java/com/aaron/sidegesture/ui/screen/home/HomeScreen.kt) 增加顶部触钮分组、空列表添加入口、开关、动作摘要、颜色标记和 bounds 高亮。Top 分组的展开状态与 Bottom／Side 互斥，但列表数据不能混合；预览使用 Insets 感知的 `bounds()`。
- [`GestureButton.kt` 扩展](../../app/src/main/java/com/aaron/sidegesture/ktx/GestureButton.kt) 增加 Top 文案。资源文案必须区分“顶部触钮”“向下”“向左”“向右”及斜向动作。
- [`GestureButtonSettingsScreen.kt`](../../app/src/main/java/com/aaron/sidegesture/ui/screen/gesturebuttonsettings/GestureButtonSettingsScreen.kt) 增加 Top 标题和方向文案；Top 隐藏 side 复制、对齐和仅 side 可用的系统返回手势选项。长度滑块的提示改为“左／右”，并显示 Top 安全区预览。
...
- 恢复过程不能把多个 `updateData` 和 `awaitAll` 当成事务。实现时新增不进入 `Backup` 内容的恢复协调状态（建议独立 MultiProcess DataStore 或等价的跨进程持久化），至少记录 `generation`、`phase/inProgress`、当前 `serviceSession`、`blockedGenerationAck`、全部目标资源的 canonical digest／代际，以及 `commitAck`／`completeAck`；通过 [`ServiceSettingsStore`](../../app/src/main/java/com/aaron/sidegesture/feature/servicesettings/ServiceSettingsStore.kt) 形成完整跨进程握手：
  ...
  3. **门禁期间写入和校验**：只有 `BLOCKED` 或明确的无消费者路径成立后，才按同一 generation 写入 initial、advanced、gesture、action、side、bottom、top 等最终值。每个 DataStore 写入完成后读回并计算 canonical digest；通知乱序、缺失、旧 generation 或任一 digest 不匹配都保持门禁，不得让 `ServiceSettingsSnapshot` 暴露中间值。运行中的 service 继续保持窗口不可见／不可触摸、拒绝新的 `ActionRequest`，服务进程对外为 null／blocked 快照。
  4. **提交请求和服务侧代际确认**：所有目标资源读回均匹配后，写入 `phase = COMMIT_REQUESTED`、`commitGeneration = generation`、完整目标 digest 和待确认的 `commitAck`。运行中的 `ServiceSettingsStore` 必须在本进程观察到 initial、advanced、gesture、action、side、bottom、top 的全部最终值，并验证摘要与 `commitGeneration` 一致；不能只看 `inProgress = false`。验证通过后才发布一次最终快照、让 `GestureWindowManager` 一次性重挂载／更新窗口、写入带 session／generation／digest 的 `completeAck`，随后才允许解除 fail closed。
  5. **完成和无消费者路径**：主进程只在收到匹配的 `completeAck` 后写入 `phase = COMPLETE`、`inProgress = false`，并将最终 generation／digest 保留在协调状态。若恢复期间没有 service，主进程可在本地读回校验后完成该握手，但下一次 service 启动必须先读取 COMPLETE 状态、重新校验全部资源摘要，再首次发布设置快照和创建窗口；若摘要不一致则回到 fail closed 并进入 journal 恢复，不得先展示旧快照。
  6. **崩溃、重启和异常**：为进程崩溃和服务重启保留不进入备份内容的恢复 journal（至少包含 generation、原快照或可重放的恢复输入及校验摘要）。任一进程在非 COMPLETE generation 期间重启都继续 fail closed；旧 serviceSession 的 ack 不得被新 session 复用。主进程启动后优先按 journal 重放整批恢复，重放失败则用原快照回滚，只有回滚／重放并校验成功后才写入匹配的 completeAck 并解除门禁；缺少有效 journal、digest 不一致或握手超时都保持禁用并提示恢复失败，不能直接把 `inProgress` 改回 false。
  7. **乱序和覆盖保护**：generation 只能单调递增，重复完成、回退 generation、旧 digest、跨进程乱序通知均被拒绝。协调状态和 journal 不得被用户备份内容覆盖，也不随旧备份字段缺失策略清空。该门禁是架构草案，当前代码尚未实现，不能以现有 `awaitAll` 宣称具备事务性。
- 备份版本低于支持 Top 的版本时，导入只恢复已知字段并显示“未包含顶部触钮”的可理解提示；备份中出现非法 position、越界 start/end 或重复 id 时拒绝对应 Top 项并记录可诊断原因，不静默转为其它边缘。
- 备份图片、临时目录清理、Base64／ZIP 流程保持现有行为，不因 Top 新增第二套文件格式。

## 实施分解

每一阶段都必须先完成直接验证，未验证不得在 ROADMAP 标记为完成：
...
3. **窗口和命中**：实现 Insets 安全矩形、WindowLayoutParams、GestureButton.bounds、GestureWindowManager 的 Top Window 创建／更新／释放；验证状态栏、cutout、`hideGestureOnIme`、横屏和系统手势下命中 Rect 与预览一致，IME 显示时必须隐藏且不可命中。
...
6. **备份兼容与恢复门禁**：补 Backup／BackupHelper 新旧文件恢复、缺失字段保留策略、非法项拒绝和 `generation`／`phase/inProgress`／`serviceSession`／`blockedGenerationAck`／目标 digest／`commitAck`／`completeAck`／journal 协调；验证运行中的 service 先确认阻断再写入，无 service 走明确无消费者路径，恢复期间 fail closed，完成后 ServiceSettingsStore 校验全部资源并只重建一次最终快照，崩溃可重放或回滚，协调状态不被备份覆盖。
...
| 窗口安全区 | 侧边 status／cutout／IME 回归 | 底部 navigation／IME 回归 | status、cutout、mandatory gesture、横屏安全区 | Window、bounds、预览同一安全矩形，无越界 |
| `hideGestureOnIme` | 既有 IME 规则不回归 | 既有 IME 规则不回归 | `imeVisible = true` 时隐藏且不可命中，隐藏优先于 fit；消失后恢复 | `imePadding` 为零、浮动键盘或过渡期间仍无 Top 触摸／动作，恢复后仅按最新 Insets 重算 |
...
| 恢复完成确认 | `completeAck` 后恢复 | `completeAck` 后恢复 | `completeAck` 后恢复 | ServiceSettingsStore 验证全部资源后只发布一次最终快照、重挂载并写完成确认；恢复期间零用户动作、零中间可用快照，完成后恰好一次最终快照 |
| 配置变化／销毁 | 重挂载回归 | 重挂载回归 | Insets 变化重算、删除释放 | 服务重启、旋转、锁屏、无障碍销毁无残留 Window |

## 风险与待确认

- `TYPE_ACCESSIBILITY_OVERLAY`、`FLAG_LAYOUT_NO_LIMITS` 和不同 ROM 的 status bar／cutout 组合可能提供不一致的坐标原点，必须在至少一个刘海设备和一个横屏设备上记录真实 Insets 与 Window bounds。
- 系统 mandatory gesture 通常集中在左右／底部，但 OEM 可能扩展顶部保护区；Top 是否允许 `excludeSystemGestureRects`、最大长度和失败反馈需要目标设备确认。
- `GestureAngle` 的 Top 文案和 `Up`／`Down` 斜向命名存在产品解释风险。实施前应以坐标图确认“左下／右下”与动作方向，不能仅凭 Bottom 文案镜像。
- 顶部触钮与状态栏下拉、通知 shade、浏览器顶部返回手势的竞争需要真机验证；若系统优先级不可控，应缩短安全区域或禁用该区域触摸，不得强制抢占。
- Top 数量上限、默认 `width`、最小安全长度、Top 是否允许系统手势排除及首页分组排序仍待产品确认；在确认前不得写入不可逆迁移。
